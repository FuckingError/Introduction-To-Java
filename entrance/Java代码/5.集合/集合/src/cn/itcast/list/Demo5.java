 ...
 * 目的:模拟注册和登陆
 * 思想：1.拿到要求，首先在主函数中确定框架
 *     2.while(){
 *           if(){
 *           }
 *         }
 *       如果出现该种嵌套,在一个范围中循环,当满足某种条件时，执行某个语句,不满足时执行的语句应当放while循环外
 *     3.当比较的是字符串时，要用equals比较，不要用==
 *       原因：字符串名存放的是字符串所存储的地址
 * 技能：
 *     1.学会重写Object类的tostring 和 equals方法
 *     2.学会break Outer; 
 *     3.学会当封装的时候在主函数中赋值   
 *     4.学会将语句块直接重构成一个方法  alt+shift+t 
 *     5.比较某个数据是否存在于集合中有两种方式
 *       (1)使用contain
